...
parser.add_argument('-d', '--data', type=str, help='dataset to use, Forum-java, HDFS or Gowalla', default='Forum-java')
...
model_time = Model(dimensionality, hidden_size)  # Create a model
...
for count in tqdm(range(math.ceil(len(list_test) / batch_size))):
    ...
    if len(list_test[num:num + batch_size]) == batch_size:
        for file in list_test[num:num + batch_size]:
            labels_test, predicts_test ,score= evaluate(file, dict_test_file, model_time, classification, rnn_model, edge_agg, device, 'debug', epoch, hidden_size, later_model)
