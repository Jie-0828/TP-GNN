...
parser.add_argument('--seed', type=str,help='Random Seed', default=824)
parser.add_argument('--divide', type=str,help='Ratio of training set to test set', default=0.3)
...
model_time = Model(features.shape[1], hidden_size)  # Create a model
...
models = [model_time, classification]
params = []
for model in models:
...
for epoch in tqdm(range(epochs)):
    ...
    time.sleep(0.0001)
    print('----------------------EPOCH %d-----------------------' % epoch)
    model_time, classification,time_model,loss = apply_model(new_feat_data,dict_edge,train_index,labels,model_time,time_model,classification,optimizer,edge_agg,device,hidden_size)
    print('loss:', loss/len(train_index))

# Testing
predicts, labels_, predicts_score = evaluate(features,dict_edge,labels,test_index,model_time,classification,time_model,edge_agg,device,hidden_size,later_model)
