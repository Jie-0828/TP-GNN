...
class Model(nn.Module):
    ...
    """
    def __init__(self, input_size, out_size, flag):
    ...
    def forward(self, raw_features,dict_edge):
        if self.flag=='graph':
            feat_data=self.get_nodes_time_embedding_graph(raw_features,dict_edge) # Get the feature of each node after aggregation
        elif self.flag=='node':
            feat_data=self.get_nodes_time_embedding_node(raw_features,dict_edge) # Get the feature of each node after aggregation
        else:
            print('error')
    ...
    def get_nodes_time_embedding_graph(self, self_feats, dict_edge):
        all_time=0
        feat_dict = {}
    ...
    def get_nodes_time_embedding_node(self, self_feats, dict_edge):
        all_time=0
        feat_dict = self_feats.clone()
        for time, edge in dict_edge.items():
            all_time = (time + 1)
        for time, edges in dict_edge.items():
            for edge in edges:
                ...
                target = int(edge[1].data)
                source_feat=self_feats[source].clone()
                source_feat=F.normalize(source_feat.unsqueeze(dim=0))
                self_feats[target] +=source_feat.squeeze(dim=0)
                result = source_feat.data
                result[:,[-1]] = (((time + 1) / all_time))
                feat_dict[target] += result.squeeze(dim=0)
        return feat_dict